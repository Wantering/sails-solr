...
const queryBody = '_query_:{!edismax qf=\'tags_search^4 display_colours_search^2 hashtags_search^2 hashtags_exact^6 category_names_search^80 brand_name_search^2 store_names_search^1 title_search^1\' v=$qq}';
...
module.exports = (function() {
    var queryBuilder = {
        ...
        buildFiltersFromWaterlineOptions: function(options, query) {
            ...
            if (query.textValue) {
                query.set('qq=' + encodeURIComponent(query.textValue.join(' ')) );
                query.q(queryBody);
                query.qop('AND');
            }
            else {
        ...
        addFilterProperty: function(prop, propCriteria, query, collection) {
            ...
            switch (prop) {
                case 'text':
                    query.textValue = (query.textValue || []).concat(propCriteria);
                ...
                case 'where':
                    var addFields = function(criteria) {
                        ...
                        for (var fieldName in criteria) {
                            if (['and'].indexOf(fieldName) > -1) {
                                _.each(criteria[fieldName], addFields);
                                continue;
                            } else {
                    ...
                    break;
                case '!':
                    console.log('Adding negative filter', propCriteria);
                    queryFragments.push({
                        fieldName: null,
            ...
            if (rawParameters) {
                if (Array.isArray(rawParameters)) {
                    rawParameters.forEach(function(param) {
                        query.set(param);
                    })
                } else {
            ...
            switch (criteriaType) {
                ...
                case 'OR':
                    ...
                    function processFilter(orFieldName, orFieldValue) {
                        ...
                        var starter = criteriaType === 'NOT' ? '-' : '';
                        return starter + searchName + ':' + adapter.getSolrFieldFilter(orFieldName, orFieldValue, collection);
                    }
     ...
     */
        getCriteriaType: function(fieldName, fieldCriteria) {
            ...
            console.log('Field name is ' + fieldName, 'Criteria is ' + fieldCriteria);
