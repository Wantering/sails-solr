...
var fieldsHelper      = require('./solrFieldsHelper');
...
module.exports = (function () {
  ...
  var adapter = {
    ...
    registerConnection: function(connectionSettings, collections, cb) {
      console.log('Linked version of sails solr running');
    ...
    define: function(connectionName, collectionName, cb) {

      // If you need to access your private data for this collection:

      var collection = connections[connectionName].collections[collectionName];
      var settings = connections[connectionName].sailsSettings;
      
      request.getAsync('http://' + settings.host + ':' + [settings.port + settings.path, settings.core, 'schema'].join('/'), {json: true})
        .spread(function(res, json) {
          console.log('Schema found');
          if (!json || !json.schema) return cb(null, collection);

          var schema = json.schema.fields;

          // collection.attributes = fieldsHelper.mapSchema(schema, collection.attributes);
          // collection.schema = fieldsHelper.mapSchema(schema, collection.attributes);
          // settings.attributes = fieldsHelper.mapSchema(schema, collection.attributes);
   ...
   //  // Return attributes
   //  describe: function (connectionName, collection, cb) {
   ...
   //  },
    ...
    find: function (connectionName, collectionName, options, cb) {
      ...
      console.log('Searching: ', query.parameters.map(decodeURIComponent.bind(decodeURIComponent)));
